...
data_second |> glimpse()
data_second |> select(tutoring_hours, paid_work_hours, ses1) |> modelsummary::datasummary_skim()
...
avg_predictions(res, variables = list(tutoring_hours = seq(0, 5.7, 0.2)), wts = res$weights) |> 
  as_tibble() |> 
  ggplot(aes(tutoring_hours, estimate, ymin = conf.low, ymax = conf.high))+
  geom_line()+
